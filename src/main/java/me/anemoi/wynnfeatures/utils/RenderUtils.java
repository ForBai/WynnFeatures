...
public class RenderUtils {
    ...
    public static void blockEspFrame(BlockPos blockPos, double red, double green, double blue) {
        ...
        double x = d - ((IRenderManager) mc.getRenderManager()).getRenderPosX();
        double d2 = blockPos.getY();
        Minecraft.getMinecraft().getRenderManager();
        double y = d2 - ((IRenderManager) mc.getRenderManager()).getRenderPosY();
        double d3 = blockPos.getZ();
        Minecraft.getMinecraft().getRenderManager();
        double z = d3 - ((IRenderManager) mc.getRenderManager()).getRenderPosZ();
        glBlendFunc(770, 771);
        GL11.glEnable(3042);
    ...
    public static void glScissor(float x, float y, float x1, float y1, ScaledResolution sr, double animation_factor) {
        float h = y + y1;
        float h2 = (float) (h * (1d - MathHelper.clamp(animation_factor, 0, 1.0025f)));
    ...
    public static void drawBox(BlockPos pos, Color color, int mode, float partialTicks) {
        ...
        Vector3d interp = interpolateEntity(mc.getRenderViewEntity(), partialTicks);

        AxisAlignedBB bb = null;
        switch (mode) {
            case 0:
                bb = iblockstate.getBlock().getSelectedBoundingBox(iblockstate, mc.world, pos).expand(0.002f, 0.002f, 0.002f).offset(-interp.x, -interp.y, -interp.z);
                break;
            case 1:
                bb = iblockstate.getBlock().getSelectedBoundingBox(iblockstate, mc.world, pos).expand(0.002f, 0.002f, 0.002f).expand(0.9, 0, 0).offset(-interp.x, -interp.y, -interp.z);
                break;
            case 2:
                bb = iblockstate.getBlock().getSelectedBoundingBox(iblockstate, mc.world, pos).expand(0.002f, 0.002f, 0.002f).expand(-0.9, 0, 0).offset(-interp.x, -interp.y, -interp.z);
                break;
            case 3:
                bb = iblockstate.getBlock().getSelectedBoundingBox(iblockstate, mc.world, pos).expand(0.002f, 0.002f, 0.002f).expand(0, 0, 0.9).offset(-interp.x, -interp.y, -interp.z);
                break;
            case 4:
                bb = iblockstate.getBlock().getSelectedBoundingBox(iblockstate, mc.world, pos).expand(0.002f, 0.002f, 0.002f).expand(0, 0, -0.9).offset(-interp.x, -interp.y, -interp.z);
                break;
        ...
        if (camera.isBoundingBoxInFrustum(new AxisAlignedBB(bb.minX + mc.getRenderManager().viewerPosX, bb.minY + mc.getRenderManager().viewerPosY, bb.minZ + mc.getRenderManager().viewerPosZ, bb.maxX + mc.getRenderManager().viewerPosX, bb.maxY + mc.getRenderManager().viewerPosY, bb.maxZ + mc.getRenderManager().viewerPosZ))) {
            ...
            RenderGlobal.drawSelectionBoundingBox(bb, (int) ((float) color.getRed() / 255.0f), (int) ((float) color.getGreen() / 255.0f), (int) ((float) color.getBlue() / 255.0f), (int) ((float) color.getAlpha() / (mode == 0 ? 255f : 510.0f)));
    ...
    public static void drawBlockOutline(BlockPos pos, Color color, float linewidth, boolean air, int mode, float partialTicks) {
        IBlockState iblockstate = mc.world.getBlockState(pos);
        if ((air || iblockstate.getBlock() != Blocks.AIR) && mc.world.getWorldBorder().contains(pos)) {
            assert (mc.getRenderViewEntity() != null);
            Vector3d interp = interpolateEntity(mc.getRenderViewEntity(), partialTicks);
            drawBlockOutline(iblockstate.getBlock().getSelectedBoundingBox(iblockstate, mc.world, pos).expand(0.002f, 0.002f, 0.002f).offset(-interp.x, -interp.y, -interp.z), color, linewidth);
    ...
    public static void blockEsp(BlockPos blockPos, Color c, double length, double length2) {
        double x = (double) blockPos.getX() - ((IRenderManager) mc.getRenderManager()).getRenderPosX();
        double y = (double) blockPos.getY() - ((IRenderManager) mc.getRenderManager()).getRenderPosY();
        double z = (double) blockPos.getZ() - ((IRenderManager) mc.getRenderManager()).getRenderPosZ();
        GL11.glPushMatrix();
        glBlendFunc(770, 771);
        GL11.glEnable(3042);
    ...
    public static AxisAlignedBB getBoundingBox(BlockPos blockPos) {
        return mc.world.getBlockState(blockPos).getBlock().getSelectedBoundingBox(mc.world.getBlockState(blockPos), mc.world, blockPos).offset(blockPos.getX(), blockPos.getY(), blockPos.getZ());
    }
    ...
    /*Start
    The Following Code is taken from DungeonGuide Mod thanks to the author
     */
    public static void drawLine(Vec3d pos1, Vec3d pos2, Color colour, float partialTicks, boolean depth) {
    ...
    public static void drawLinesVec3(List<Vec3d> poses, Color colour, float thickness, float partialTicks, boolean depth) {
        Entity render = Minecraft.getMinecraft().getRenderViewEntity();
        BufferBuilder worldRenderer = Tessellator.getInstance().getBuffer();
        ...
        for (Vec3d pos : poses) {
            worldRenderer.pos(pos.x + 0.5, pos.y + 0.5, pos.z + 0.5).endVertex();
        }
        Tessellator.getInstance().draw();
    ...
    public static void drawLines(java.util.List<BlockPos> poses, Color colour, float thickness, float partialTicks, boolean depth) {
        Entity render = Minecraft.getMinecraft().getRenderViewEntity();
        BufferBuilder worldRenderer = Tessellator.getInstance().getBuffer();
    ...
    public static void renderCircleLine(Vec3d pos, float radius, Color color, float partialTicks, float lineWidth) {
        Entity render = Minecraft.getMinecraft().getRenderViewEntity();
        BufferBuilder worldRenderer = Tessellator.getInstance().getBuffer();
        double realX = render.lastTickPosX + (render.posX - render.lastTickPosX) * partialTicks;
        double realY = render.lastTickPosY + (render.posY - render.lastTickPosY) * partialTicks;
        ...
        for (int i = 0; i < 360; i++) {
            ...
            worldRenderer.pos(pos.x + x, pos.y, pos.z + z).endVertex();
        }
        Tessellator.getInstance().draw();
        GlStateManager.translate(realX, realY, realZ);
        GlStateManager.disableBlend();
    ...
    public static void renderCylinder(Vec3d pos, float radius, float height, Color color, float partialTicks) {
        Entity render = Minecraft.getMinecraft().getRenderViewEntity();
        BufferBuilder worldRenderer = Tessellator.getInstance().getBuffer();
        double realX = render.lastTickPosX + (render.posX - render.lastTickPosX) * partialTicks;
        double realY = render.lastTickPosY + (render.posY - render.lastTickPosY) * partialTicks;
    ...
    public static void renderWaypointText(String str, double X, double Y, double Z, float partialTicks) {
        ...
        Entity viewer = WynnFeatures.mc.getRenderViewEntity();
        ...
        GlStateManager.rotate(-WynnFeatures.mc.getRenderManager().playerViewY, 0.0F, 1.0F, 0.0F);
        GlStateManager.rotate(WynnFeatures.mc.getRenderManager().playerViewX, 1.0F, 0.0F, 0.0F);
        GlStateManager.translate(0, -0.25f, 0);
        GlStateManager.rotate(-WynnFeatures.mc.getRenderManager().playerViewX, 1.0F, 0.0F, 0.0F);
        GlStateManager.rotate(WynnFeatures.mc.getRenderManager().playerViewY, 0.0F, 1.0F, 0.0F);

        drawNametag(ChatFormatting.YELLOW.toString() + Math.round(dist) + " blocks");
    ...
    public static void renderWaypointTextEasy(String str, Vec3d pos, float partialTicks) {
        GlStateManager.alphaFunc(516, 0.1F);
        ...
        double y = pos.y - viewerY - viewer.getEyeHeight();
        double z = pos.z - viewerZ;
